function [sol,b,obj] = primal_svm_block(linear,Y,lambda,pred_old,opt)
...
% modified by Liefeng Bo on 01/04/2011 in University of Washington
  ...
  if ~isfield(opt,'iter_max_Newton'),   opt.iter_max_Newton = 30;          end;  
  ...
  if  linear 
    ...
    if ~opt.cg
      [sol,obj] = primal_svm_linear   (Y,lambda,pred_old,opt);
    else
      [sol,obj] = primal_svm_linear_cg(Y,lambda,pred_old,opt);
    end;
...
function  [w,obj] = primal_svm_linear(Y,lambda,pred_old,opt) 
% -------------------------------
...
function  [w, obj] = primal_svm_linear_cg(Y,lambda,pred_old,opt)
% -----------------------------------------------------
...
% -----------------------------------------------------
  ...
  w = zeros(d+1,1); % The last component of w is b.
  iter = 0;
  out = ones(n,1) - Y.*pred_old; % Vector containing 1-Y.*(X*w)
