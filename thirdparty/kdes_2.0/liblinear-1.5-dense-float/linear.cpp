		...
		for(s=0;s<active_size;s++)
		{
			...
			if(Ai > 0)
			{
				...
				for(j=0; j < w_size ;j++)
				{
					...
					for(m=0;m<active_size_i[i];m++)
						G[m] += w_i[alpha_index_i[m]]*(double)(xi[j]);
				}
				...
				for(j = 0; j < w_size; j++)
				{
					double *w_i = &w[(j)*nr_class];
					for(m=0;m<nz_d;m++)
						w_i[d_ind[m]] += d_val[m]*(double)xi[j];
				}
#else
				xi = prob->x[i];
				while(xi->index != -1)
	...
	double Cp, double Cn)
{
	...
	for(j=0; j<w_size; j++)
	{
		...
		for(k = 0; k < l; k++)
		{
			double val = (double)x[k];
			x[k] *= (float)prob_col->y[k]; // x->value stores yi*xij
			...
			for(num_linesearch=0; num_linesearch < max_num_linesearch; num_linesearch++)
			{
				...
				if(num_linesearch == 0)
				{
					...
					for(k = 0; k < l; k++)
					{
						...
						double b_new = b[k] + d_diff*(double)x[k];
						b[k] = b_new;
						if(b_new > 0)
	...
	for(j=0; j<w_size; j++)
	{
		...
		for(int k = 0; k < l; k++)
		{
			x[k] *= (float)prob_col->y[k]; // restore x->value
		}
#else
		x = prob_col->x[j];
		while(x->index != -1)
	...
	while(iter < max_iter)
	{
		...
		for(s=0; s<active_size; s++)
		{
			...
			for(k = 0; k < l; k++)
			{
				...
				double tmp1 = (double)x[k]/(1+exp_wTxind);
			...
			for(num_linesearch=0; num_linesearch < max_num_linesearch; num_linesearch++)
			{
				...
				if(x_min >= 0)
				{
					...
					if(min(appxcond1,appxcond2) <= 0)
					{
						...
						for(k = 0; k < l; k++)
						{
							exp_wTx[k] *= exp(d*(double)x[k]);
						}
#else
						x = prob_col->x[j];
						while(x->index != -1)
						{
							exp_wTx[x->index] *= exp(d*(double)(x->value));
							x++;
						}
#endif
						break;
				...
				for(k = 0; k < l; k++)
				{
					double exp_dx = exp(d*(double)x[k]);
...
struct model *load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		if(strcmp(cmd,"solver_type")==0)
		{
			fscanf(fp,"%80s",cmd);
			int ii;
			for(ii=0;solver_type_table[ii];ii++)
			{
				if(strcmp(solver_type_table[ii],cmd)==0)
				{
					param.solver_type=ii;
					break;
				}
			}
...
#ifdef _DENSE_REP
int predict_values(const struct model *model_, const float *x, double *dec_values)
#else
...
#endif
{
	...
	{
	        for(i=0;i<nr_w;i++)
        		  dec_values[i] += w[k*nr_w+i]*(double)lx[k];
	}
#else
...
#ifdef _DENSE_REP
int predict_probability(const struct model *model_, const float *x,double* prob_estimates)
#else
