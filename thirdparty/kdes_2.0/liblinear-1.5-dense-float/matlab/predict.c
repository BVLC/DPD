...
#ifdef _DENSE_REP
//void read_sparse_instance(const mxArray *prhs, int index, float *x, int feature_number, double bias)
void read_sparse_instance(const mxArray *prhs, int index, float *x, int feature_number, int testing_instance_number, double bias, bool transpose=false)
#else
...
#ifdef _DENSE_REP
	samples = (float*)mxGetPr(prhs);
	low = feature_number*index ;
	for(i=0; i < feature_number ; i++)
	  if (!transpose) {
	    x[i] = (float)samples[low + i];
	  } else {
	    x[i] = (float)samples[i*testing_instance_number+index];
	  }

	if(bias >= 0)
	  x[feature_number] = (float)bias;
#else
	int j, high;
	mwIndex *ir, *jc;
...
*/
...
#ifdef _DENSE_REP
	x = Malloc(float, feature_number+1);
#else
	...
	for(instance_index=0;instance_index<testing_instance_number;instance_index++)
	{
		...
		read_sparse_instance(prhs[1], instance_index, x, feature_number, testing_instance_number, model_->bias, !col_format_flag);
